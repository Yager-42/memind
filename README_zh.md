...
最快的体验方式是使用 Docker Compose。它会一次性启动本地 `memind-server` API 和
React 管理 UI，不需要在宿主机安装 Java、Maven、Node.js 或 pnpm。
...
默认情况下，`memind-server` 会把 SQLite 数据和 fallback file vector store 存放在
名为 `memind-data` 的 Docker volume 中，容器内挂载路径为 `/app/data`。这套 Compose
配置面向本地开发和数据检查；管理 UI 没有认证能力，请不要直接暴露到公网。

### Java quickstart 示例

如果你希望从源码运行，可以使用默认的 **纯 Java + OpenAI + SQLite** 路径。
